...
一个 [ZCode](https://z.ai) 插件：当一轮对话的耗时达到阈值（默认 5 分钟，可调）时，通过 [Qmsg 酱](https://qmsg.zendee.cn)给你的 QQ 发送私聊通知。纯 hook 实现，AI 无需配合，不干扰正常对话。
...
- hook 注册在**会话启动时快照**：安装/卸载/更新插件后需要新会话（或重启 ZCode）才生效；但 `config.json` 的 key/阈值是每次触发时读取的，改完即生效
