...
3. **看门狗（watchdog.mjs）** — 由 Windows 计划任务每分钟调起的单次巡检（进程运行在 ZCode 之外，不受会话结束影响）：检查各未完成回合在其会话日志中是否已停止活动——状态残留且日志沉默超过失联阈值（默认 5 分钟）→ 发送"⚠️ 任务中断"（每回合最多一次）。ZCode 已关闭时不报警
...
- **中断通知的延迟 = 失联阈值 + 至多 1 分钟巡检间隔**；会话在 ZCode 运行日志中查不到活动时不报警（宁缺勿滥）；ZCode 关闭期间不报警，恢复后补报
- **hook 派生的后台进程会被 ZCode 回收**（Windows 作业对象行为），因此看门狗必须走计划任务而非 detached 子进程——这是实测得出的平台限制
